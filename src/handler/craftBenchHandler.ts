...
export const createCraftBench = async (
  meta: any,
  folioConfig?: any,
  isRecentConfig?: boolean
) => {
  try {
    ...
    if (!folioConfig && isRecentConfig) {
      ...
      throw new Error(
        'Folio configuration is required to create a craft bench.'
      )
    }
...
export const downloadCode = async (craftId: string) => {
  try {
    ...
    const response = await fetch(
      `${BACKEND_URL}/craftbench/download/${craftId}`,
      {
        method: 'GET',
        headers: headers
      }
...
export const publishFolio = async (craftId: string) => {
  try {
    ...
    const response = await fetch(
      `${BACKEND_URL}/craftbench/publish/${craftId}`,
      {
        method: 'POST',
        headers: headers
      }
...
export const uploadResume = async (file: File) => {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/craftbench/upload`, {
      method: 'POST',
      headers: headers,
      body: formData
    })
